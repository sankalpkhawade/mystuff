...
export class CarListService {
  ...
  getFoodItem(id:string):Observable<Vehicle>{
    let headers = new HttpHeaders();
    headers = headers.set('Authorization', 'Bearer ' + this.userService.getToken());  
    return this.httpClient.get<Vehicle>(environment.baseUrl+'vehicle/veh/'+id,{headers});
  }
  modifyItem(vehicle:Vehicle):Observable<any>{
  let headers = new HttpHeaders();
  headers = headers.set('Authorization', 'Bearer ' + this.userService.getToken()); 
  return this.httpClient.put<Vehicle>(environment.baseUrl+'vehicle',vehicle,{headers});
  }
